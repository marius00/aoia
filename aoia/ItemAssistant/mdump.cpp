...
LONG MiniDumper::TopLevelFilter( struct _EXCEPTION_POINTERS *pExceptionInfo )
{
	...
	if (hDll)
	{
		...
		if (pDump)
		{
			...
			if (::MessageBox( NULL, _T("The application crashed. Would you like to save a diagnostic file that you can send to the developers?"), m_szAppName, MB_YESNO ) == IDYES)
			{
				// create the file
				...
				if (hFile!=INVALID_HANDLE_VALUE)
				{
					...
					if (bOK)
					{
						_stprintf( szScratch, _T("Saved dump file to '%s'. Please send this file to morten.fjeldstad@gmail.com. Thanks!"), szDumpPath );
						szResult = szScratch;
